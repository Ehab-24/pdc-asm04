...
if __name__ == '__main__':
    ...
    config = master.Config(docs=None, output_filename='output.txt', num_mappers=1, num_reducers=1)
    if args.task == 1:
        config.docs = task1.generate_docs(db)
        mapFunc = task1.map
        reduceFunc = task1.reduce
    elif args.task == 2:
        config.docs = task2.generate_docs(db)
        mapFunc = task2.map
        reduceFunc = task2.reduce
    elif args.task == 3:
        config.docs = task3.generate_docs(db)
        mapFunc = task3.map
